/**
 * Wave 58 leftover after #275 — Contig pass warning for Red seat. Tests-only.
 */
...
describe('Wave 58 contig — pass warning player2', () => {
  it('uses Red consecutivePasses for (1/3 passes)', async () => {
    const { status } = await seed({
      currentPlayer: 'player2',
      consecutivePasses: { player1: 0, player2: 1 },
      phase: 'rolling',
    });
    expect(status.querySelector('.contig-status')?.textContent).toMatch(
      /Red's turn.*\(1\/3 passes\)/
    );
  });
});
