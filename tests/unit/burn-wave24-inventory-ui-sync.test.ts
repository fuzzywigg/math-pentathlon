/**
 * Wave 24 — inventory chrome sync after legal ledger changes.
 * Second lane after #127. Distinct from #130 hex/poly/storage/owl UI and from
 * wave 22 tutorial/reject chrome. Vitest-only. No product inventing.
 */
...
describe('Wave 24 inventory-ui — Par hand count after place', () => {
...
describe('Wave 24 inventory-ui — Sum hand tiles after place', () => {
...
describe('Wave 24 inventory-ui — Stars hand + score chrome', () => {
  it('renderPlayerHand reflects post-place hand; scores mount', () => {
    ...
    expect(
      scores.querySelector('.stars-score, .stars-scores') || scores
    ).toBeTruthy();
    expect(scores.textContent?.length).toBeGreaterThan(0);
  });
});

describe('Wave 24 inventory-ui — Hex-a-Gone bank block-count text', () => {
  it('renderBoard bank count drops after placeBlock', () => {
    ...
    expect(
      counts.some((t) => t.includes(`${forBank.bank.triangle} left`))
    ).toBe(true);
  });
});

describe('Wave 24 inventory-ui — Frac / Pinball / Fab score chrome', () => {
...
describe('Wave 24 inventory-ui — Kings supply chrome after place', () => {
...
describe('Wave 24 inventory-ui — Pent piece selector tracks available', () => {
...
describe('Wave 24 inventory-ui — Ramrod rod strip after place', () => {
...
describe('Wave 24 inventory-ui — Calla pit chrome after sow', () => {
