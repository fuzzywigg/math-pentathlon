/**
 * Wave 58 leftover after #275 — Contig placing phase status copy. Tests-only.
 */
...
describe('Wave 58 contig — placing status copy', () => {
  it('seeded placing phase shows click-valid-cell instruction', async () => {
    const { status } = await seed({ phase: 'placing' });
    expect(status.querySelector('.contig-status')?.textContent).toMatch(
      /Click a valid cell to place your chip/
    );
  });
});
