...
describe('Wave 58 frac inject — answer-box correct border', () => {
  it('includes .frac-answer-box.correct border-color #4caf50 leftover', () => {
    injectFracFactStyles();
    const css = document.getElementById('frac-fact-styles')!.textContent || '';
    expect(css).toMatch(/\.frac-answer-box\.correct\s*\{[\s\S]*?border-color:\s*#4caf50/);
  });
});
