...
describe('Wave 64 core owl — format game-start-return-1', () => {
  ...
  it('selects game-start-return-1 at gamesPlayed gte-5', () => {
    for (const m of owlMessages.getMessagesByCategory('game:start')) {
      if (m.id !== 'game-start-return-1') storage.markMessageSeen(m.id);
    }
    const msg = owlMessages.selectMessage('game:start', {
      gameName: 'Contig',
      gamesPlayedThisGame: 5,
    });
    expect(msg!.id).toBe('game-start-return-1');
    expect(msg!.text).toBe(
      'Contig again! I can see you really like this one!'
    );
  });
