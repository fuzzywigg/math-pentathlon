...
describe('Wave 67 core owl — format win-generic-5', () => {
  ...
  it('selects win-generic-5 when higher-priority wins are seen', () => {
    for (const m of owlMessages.getMessagesByCategory('game:end')) {
      if (m.id !== 'win-generic-5') storage.markMessageSeen(m.id);
    }
    const msg = owlMessages.selectMessage('game:end', { playerWon: true, gamesPlayedThisGame: 4, winStreak: 1 });
    expect(msg!.id).toBe('win-generic-5');
    expect(msg!.text).toBe(
      'Champion! Want to try for another win?'
    );
  });
