/**
 * Wave 53 leftover after #235 — Fab answer matchable class. Tests-only.
 */
...
describe('Wave 53 fab — answer matchable', () => {
  it('adds .fab-answer-matchable for matching result', () => {
    ...
    const matchable = el.querySelectorAll('.fab-answer-matchable');
    expect(matchable.length).toBeGreaterThan(0);
    expect(
      [...matchable].some((n) => (n as HTMLElement).dataset.answerId === found!.ans)
    ).toBe(true);
  });
});
