/**
 * Wave 58 leftover after #275 — Contig scores shell class. Tests-only.
 */
...
describe('Wave 58 contig — scores shell class', () => {
  it('mounts .contig-scores with p1/p2 score children', async () => {
    const { board } = await seed({});
    expect(board.querySelector('.contig-scores')).toBeTruthy();
    expect(board.querySelector('.contig-score.contig-score-p1')).toBeTruthy();
    expect(board.querySelector('.contig-score.contig-score-p2')).toBeTruthy();
  });
});
