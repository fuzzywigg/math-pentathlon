/**
 * Wave 65 leftover after tip/#313 — Kings renderer piece border-radius 50%. Tests-only.
 */
...
describe('Wave 65 kings — renderer piece border-radius 50', () => {
  it('piece border-radius 50%', () => {
    renderBoard(createInitialBoard());
    const css = document.querySelector('style[data-board-styles]')?.textContent ?? '';
    expect(css).toMatch(/\.piece\s*\{[^}]*border-radius:\s*50%/s);
  });
});
