/**
 * Wave 15 — vertical rules-phase / illegal-sequence / getValid* matrix.
 * Distinct from waves 7–14 (win-draw-AI, secondary UI, tutorial wiring, controllers).
...
import {
  getValidMoves as queensValidMoves,
...
import {
  createInitialGameState as createKingsBoard,
  PLAYER1_KING_START,
} from '../../src/games/kings-quadraphages/board';
import {
  createInitialGameState as createKingsPhase,
  selectKing,
  moveKing,
  placeQuadraphage,
...
describe('Wave 15 rules-phase — Hex-a-Gone empty / wrong-phase guards', () => {
...
describe('Wave 15 rules-phase — Juggle wrong-phase / empty preview', () => {
...
describe("Wave 15 rules-phase — Pent'Em In placements / selection guards", () => {
...
describe('Wave 15 rules-phase — Prime Gold rolling / miss / history', () => {
...
describe('Wave 15 rules-phase — Remainder Islands preview / skip / select guards', () => {
...
describe('Wave 15 rules-phase — Calla animating/gameOver pit gates', () => {
...
describe('Wave 15 rules-phase — Star Track draw/select identity + progress', () => {
...
describe('Wave 15 rules-phase — Fab pipeline illegal matrix', () => {
...
describe('Wave 15 rules-phase — Stars / Par / Ramrod / Kwatro / Sum / FIAR / Contig / Quiz', () => {
...
describe('Wave 15 rules-phase — Hex / Queens / Kings getValid* + illegal', () => {
  it('Hex: getValidMoves empties after win; occupied/OOB illegal; remake identity', () => {
    let state = createHex(3);
    expect(hexValidMoves(state)).toHaveLength(9);
    expect(hexIsValid(state, { row: 0, col: 0 })).toBe(true);
    expect(hexIsValid(state, { row: -1, col: 0 })).toBe(false);
    state = hexMove(state, { row: 0, col: 0 });
    expect(hexIsValid(state, { row: 0, col: 0 })).toBe(false);
    expect(hexMove(state, { row: 0, col: 0 })).toBe(state);

    // Force a finished board: winner set → no valid moves / illegal place
    const finished = { ...state, winner: 'player1' as const };
    expect(hexValidMoves(finished)).toEqual([]);
    expect(hexIsValid(finished, { row: 1, col: 1 })).toBe(false);
    expect(hexMove(finished, { row: 1, col: 1 })).toBe(finished);
  });

  it('Queens: empty-cell getValidMoves []; opponent piece []; illegal makeMove identity', () => {
    const state = createQueens();
    // Center throne starts empty → no piece → empty getValid*
    expect(queensValidMoves(state, { ring: 0, position: 0 })).toEqual([]);
    expect(queensHasMoves(state)).toBe(true);

    let own: { ring: number; position: number } | null = null;
    for (const [key, cell] of state.cells) {
      if (cell.piece?.player === 'player1') {
        const [ringStr, posStr] = key.split('-');
        own = { ring: Number(ringStr), position: Number(posStr) };
        break;
      }
    }
    expect(own).not.toBeNull();
    const moves = queensValidMoves(state, own!);
    expect(Array.isArray(moves)).toBe(true);

    const selected = queensSelect(state, own!);
    if (moves.length > 0) {
      expect(selected.selectedPiece).not.toBeNull();
      // Illegal destination (same cell) is identity
      expect(queensMove(selected, own!, own!)).toBe(selected);
    } else {
      expect(selected.selectedPiece).toBeNull();
    }

    let opp: { ring: number; position: number } | null = null;
    for (const [key, cell] of state.cells) {
      if (cell.piece?.player === 'player2') {
        const [ringStr, posStr] = key.split('-');
        opp = { ring: Number(ringStr), position: Number(posStr) };
        break;
      }
    }
    expect(opp).not.toBeNull();
    expect(queensValidMoves(state, opp!)).toEqual([]);
    expect(queensSelect(state, opp!).selectedPiece).toBeNull();
  });

  it('Kings rules: getValidKingMoves opening; OOB/stay illegal; phase wrong-turn identity', () => {
    const board = createKingsBoard();
    const p1Moves = getValidKingMoves(board, 'player1');
    expect(p1Moves.length).toBeGreaterThan(0);
    expect(isValidKingMove(board, 'player1', PLAYER1_KING_START)).toBe(false);
    expect(isValidKingMove(board, 'player1', { row: -1, col: 0 })).toBe(false);
    expect(isValidKingMove(board, 'player1', { row: 0, col: 3 })).toBe(true);
    expect(isValidKingMove(board, 'player1', { row: 0, col: 0 })).toBe(false);
    expect(getValidQuadraphagePlacements(board).length).toBe(BOARD_EMPTY_CELLS());
    expect(isValidQuadraphagePlacement(board, { row: 0, col: 0 })).toBe(true);
    expect(isValidQuadraphagePlacement(board, PLAYER1_KING_START)).toBe(false);
    expect(canCompleteTurn(board, 'player1')).toBe(true);

    // Phase machine: wrong-phase place/select are identity
    let phase = createKingsPhase();
    expect(placeQuadraphage(phase, { row: 3, col: 3 })).toBe(phase); // still moveKing
    phase = selectKing(phase);
    expect(phase.selectedKingPosition).not.toBeNull();
    phase = moveKing(phase, { row: 2, col: 5 });
    expect(phase.turnPhase).toBe('placeQuadraphage');
    expect(selectKing(phase)).toBe(phase); // wrong phase
    const placements = getValidQuadraphagePlacements({
      board: phase.board,
      player1Supply: phase.player1Supply,
      player2Supply: phase.player2Supply,
    });
    expect(placements.length).toBeGreaterThan(0);
  });
});

function BOARD_EMPTY_CELLS(): number {
  // 9x9 minus 2 kings
  return 81 - 2;
}
