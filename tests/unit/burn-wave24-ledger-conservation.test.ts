/**
 * Wave 24 — multi-step inventory conservation invariants.
 * Second lane after #127. Distinct from #130 storage/owl and wave 19 opening
 * invariants. Vitest-only. No product inventing.
 */
...
describe('Wave 24 conservation — Hex-a-Gone bank + filled cells', () => {
...
describe('Wave 24 conservation — Calla TOTAL_CUBES across two sows', () => {
...
describe('Wave 24 conservation — Prime chips + claimed cells', () => {
  it('STARTING_CHIPS×2 = remaining chips + owned cells after place', () => {
    ...
    for (let seed = 0; seed < 40 && !done; seed++) {
      ...
      const opening = PRIME_CFG.STARTING_CHIPS * 2;
      ...
      const remaining = state.playerChips.player1 + state.playerChips.player2;
...
describe('Wave 24 conservation — Stars hand+deck card accounting', () => {
...
describe('Wave 24 conservation — Par bases occupancy vs hand ids', () => {
...
describe('Wave 24 conservation — Fab used bars + claimed answers', () => {
  it('after executeMove: used+unused bars = total; claimed+free answers = total', () => {
    ...
    const unused = [...state.fractionBars.values()].filter(
      (x) => !x.used
    ).length;
...
describe('Wave 24 conservation — FIAR chipsPlaced + occupied nodes', () => {
  it('chipsPlaced sum equals occupied board nodes during placement', () => {
    ...
    for (const id of ids) {
      ...
      expect(state.chipsPlaced.player1 + state.chipsPlaced.player2).toBe(
        occupied
      );
    }
    expect(
...
describe('Wave 24 conservation — Star Track progress bounds', () => {
