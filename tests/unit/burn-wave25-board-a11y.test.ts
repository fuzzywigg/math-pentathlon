 ...
 * Deepens beyond board-a11y.test.ts. Distinct from wave 24 inventory and wave 23 owl.
...
describe('Wave 25 board-a11y — keyboard bindings + focus restore', () => {
  it('bindGridNavigation moves focus with arrows and ignores non-grid targets', () => {
    markBoardAsGrid(root);
    const a = gridCell(0, 0);
    const b = gridCell(0, 1);
    root.append(a, b);
    ...
    const ignored = vi.fn();
    a.addEventListener('keydown', ignored);
