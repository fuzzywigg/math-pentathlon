...
describe('Wave 67 core owl — return-morning-1 conditions', () => {
  it('pins timeOfDay morning + normal priority', () => {
    const msg = owlMessages
      .getMessagesByCategory('app:return')
      .find((m) => m.id === 'return-morning-1')!;
    expect(msg.priority).toBe('normal');
    expect(msg.conditions).toEqual([{ type: 'timeOfDay', value: 'morning' }]);
  });
});
