...
describe('Wave 67 core owl — return-streak-big-1 conditions', () => {
  it('pins streak gte-7 + high priority', () => {
    const msg = owlMessages
      .getMessagesByCategory('app:return')
      .find((m) => m.id === 'return-streak-big-1')!;
    expect(msg.priority).toBe('high');
    expect(msg.conditions).toEqual([{ type: 'streak', value: 7, operator: 'gte' }]);
  });
});
