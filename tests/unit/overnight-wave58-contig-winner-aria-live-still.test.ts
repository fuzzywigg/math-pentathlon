/**
 * Wave 58 leftover after #275 — Contig winner path keeps aria-live. Tests-only.
 */
...
describe('Wave 58 contig — winner aria-live', () => {
  it('markStatusLive still on status container under winner', async () => {
    const { status } = await seed({
      winner: 'player2',
      scores: { player1: 1, player2: 7 },
      phase: 'gameOver',
    });
