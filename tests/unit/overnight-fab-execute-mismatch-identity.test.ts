/**
 * Overnight TOKENMAXX — executeMove result mismatch identity leftovers. Tests-only.
 */
...
describe('Overnight fab — execute mismatch', () => {
  it('wrong answer id leaves confirmingMove', () => {
    ...
    const wrong = [...state.answerBars.keys()].find((id) => id !== found!.ans)!;
    const next = executeMove(state, wrong);
