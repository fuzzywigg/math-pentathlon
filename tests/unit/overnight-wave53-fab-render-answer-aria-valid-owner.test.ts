/**
 * Wave 53 leftover after #235 — Fab answer aria valid/owner. Tests-only.
 */
...
describe('Wave 53 fab — answer aria', () => {
  it('matchable aria includes valid placement; claimed includes Blue owner', () => {
    ...
    const other = [...state.answerBars.keys()].find((id) => id !== found!.ans)!;
    const answers = new Map(state.answerBars);
    answers.set(other, { ...answers.get(other)!, claimedBy: 'player1' });
    state = { ...state, answerBars: answers };
    const el = renderAnswerBoard(state, () => undefined);
    const matchLabel =
      (el.querySelector(`[data-answer-id="${found!.ans}"]`) as HTMLElement).getAttribute('aria-label') || '';
    const claimedLabel =
      (el.querySelector(`[data-answer-id="${other}"]`) as HTMLElement).getAttribute('aria-label') || '';
    expect(matchLabel.toLowerCase()).toMatch(/valid|placement/);
    expect(claimedLabel).toMatch(/Blue/);
  });
});
