/**
 * Wave 20 — board bounds / empty-structure geometry for existing games.
 * Distinct from wave 18 transforms and wave 19 lookup/occupancy.
...
describe('Wave 20 board-bounds — Hex empty board + position guards', () => {
  ...
  it('createEmptyBoard(n) is n×n of null for small n', () => {
    ...
    expect(
      board.every((row) => row.length === 3 && row.every((c) => c === null))
    ).toBe(true);
  });
  ...
  it('corner cells have fewer than 6 neighbors; center has 6', () => {
    const size = 5;
    ...
    expect(getNeighbors({ row: 2, col: 2 }, size)).toHaveLength(6);
  });
});

describe('Wave 20 board-bounds — Hex-a-Gone radius inventory', () => {
  it('radius-3 board has 37 cells; all listed cells are valid', () => {
...
describe('Wave 20 board-bounds — Queens createBoard geometry', () => {
  it('createBoard cell count equals sum of cellsInRing over rings', () => {
    ...
    expect(QG_CFG.NUM_RINGS).toBe(6);
...
describe('Wave 20 board-bounds — Kings position/supply geometry', () => {
...
describe('Wave 20 board-bounds — Contig createBoard uniqueness', () => {
  it('default createBoard maps unique values for every grid cell', () => {
    ...
    for (let row = 0; row < CONTIG_CFG.GRID_ROWS; row++) {
      for (let col = 0; col < CONTIG_CFG.GRID_COLS; col++) {
        ...
        seen.add(value!);
        expect(cells.get(value!)).toEqual({
          value,
          owner: null,
          row,
          col,
