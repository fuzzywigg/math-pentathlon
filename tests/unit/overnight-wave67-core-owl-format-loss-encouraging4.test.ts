...
describe('Wave 67 core owl — format loss-encouraging-4', () => {
  ...
  it('selects loss-encouraging-4 when other losses are seen', () => {
    for (const m of owlMessages.getMessagesByCategory('game:end')) {
      if (m.id !== 'loss-encouraging-4') storage.markMessageSeen(m.id);
    }
    const msg = owlMessages.selectMessage('game:end', { playerWon: false });
    expect(msg!.id).toBe('loss-encouraging-4');
